 ...
 * 3.【规律】V4 强在推理和方案,弱在无工具;agent 强在有工具,弱在推理深度
 * 4.【一分为二】找 V4 获得深度方案,但消耗时间;不找 V4 快速响应,但可能方案不够优雅
 * 
 * 决策规则:
 * - 简单任务(<10行代码/纯信息查询)→ agent 自己做,不问 V4
 * - 复杂架构/策略/方案 → 问 V4,获取辩证分析
 * - 代码执行/文件操作 → agent 自己做,V4 无工具
 * 
 * 环境变量:
 * - DS_AGENT_NAME: agent 名称(默认: DSH Agent)
 * - DS_RUNTIME_ENV: 运行环境(默认: DSH (DeepSeek Harness))
 */
